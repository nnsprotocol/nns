...
const resolvers = {
  Query: {
    async getPriceCurve(_, {}) {
      ...
      const res = registrar.getPriceCurve()
      return res
    },
    async getEthPrice(_, {}) {
      const registrar = getRegistrar()
      const res = registrar.getEthPrice()
      return res
    },
    async getPrice(_, { label, duration }) {
      const registrar = getRegistrar()
      return registrar.getPrice(label)
    },
    ...
    //   console.log('getRentPriceAndPremium -> duration')
    //   const registrar = getRegistrar()
    //   const res= registrar.getRentPriceAndPremium(label, duration, block)
    //   console.log('getRentPriceAndPremium', await res, duration);
    //   return res;
    // },
    ...
    //   console.log('getTimeUntilPremium')
    //   const registrar = getRegistrar()
  ...
  Mutation: {
    ...
    async register(_, { label, secret }) {
      const registrar = getRegistrar()
      const tx = await registrar.register(label, secret)
    ...
    async getDomainAvailability(_, { name }) {
      ...
      try {
        ...
        if (modeNames[state] === 'Owned') {
          owner = await ens.getOwner(`${name}.⌐◨-◨`) // FIXME
        }
