  ...
  IERC721Receiver
{
  ...
  function registerAndWrapETH2LD(
    ...
    uint256 tokenId = uint256(keccak256(bytes(label)));
    // registrarExpiry = registrar.register(tokenId, address(this));
  ...
  function wrap(
    ...
    bytes32 parentNode = name.namehash(offset, 0);
    ...
    )
  {
    uint64 oldExpiry;
    (owner, fuses, oldExpiry) = getData(uint256(node));
    uint64 maxExpiry = 0; //uint64(registrar.nameExpires(uint256(labelhash)));
