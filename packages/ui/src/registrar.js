...
function checkArguments({ registryAddress, ethAddress, provider }) {
...
export default class Registrar {
  constructor({ registryAddress, ethAddress, controllerAddress, provider }) {
    checkArguments({
      registryAddress,
      ethAddress,
      provider
  ...
  async getPriceCurve() {
    ...
    try {
      ...
      console.log('fall back to leaner')
      return 'linear'
    }
  }

  async getRentPrices(labels) {
  ...
  async register(label, secret) {
    ...
    const account = await getAccount()
    const price = await this.getPrice(label)
    ...
    if (parseInt(resolverAddr, 16) === 0) {
      ...
      return permanentRegistrarController.register(label, account, secret, {
        value: priceWithBuffer,
        gasLimit
      })
    } else {
  ...
  async submitProof(name, parentOwner) {
    ...
    if (data.length === 0) {
      ...
      if (!isOld && owner === user) {
        const resolverAddress = await this.getAddress('resolver.⌐◨-◨') // FIXME
